...
(function ($, window, undefined) {
  ...
  $.Calendario.defaults = {
    ...
    */
    ...
    feedParser: './feed/',
    zone: '00:00',
    // Ex: IST zone time is '+05:30' by default it is GMT, Sign is important.
    format: 'MM-DD-YYYY',
  ...
  $.Calendario.prototype = {
    ...
    _parseDataToDay: function _parseDataToDay(data, day, other) {
      var content = '';
      if (!other) {
        if (Array.isArray(data)) content = this._convertDayArray(data, day);else content = this._wrapDay(data, day, true);
      } else {
        if (!Array.isArray(data)) data = [data];
        for (var i = 0; i < data.length; i++) {
          if (this.month != 1 && day >= data[i].startDate && day <= data[i].endDate) content += this._wrapDay(data[i], day, true);else if (this.month == 1 && day >= data[i].startDate) {
            if (data[i].endDate && day <= data[i].endDate) content += this._wrapDay(data[i], day, true);else if (!data[i].endDate) content += this._wrapDay(data[i], day, true);
          }
        }
      }
      return content;
    ...
    _getBody: function _getBody() {
      ...
      for (var i = 0; i < 7; i++) {
        ...
        for (var j = 0; j <= 6; j++) {
          ...
          if (past) cellClasses += 'fc-past ';else cellClasses += 'fc-future ';
    ...
    ***** PUBLIC METHODS *****
    **************************/
    option: function option(_option, value) {
      if (value) this.options[_option] = value;else return this.options[_option];
    },
    ...
    setData: function setData(caldata, clear) {
      ...
      if (clear) this.caldata = caldata;else $.extend(this.caldata, caldata);
  ...
  $.fn.calendario = function (options) {
    ...
    if (typeof options === 'string') {
      ...
      this.each(function () {
        if (instance) instance._init();else instance = $.data(this, 'calendario', new $.Calendario(options, this));
      });
    }
